...
const SCRIPT_LINES = [
  ...
  "var CARD_ERROR = null;",
  "var KNOWN_KINDS = [];",
  "var HEADER_NAMES = ['content-type', 'cache-control', 'x-index-generated-at', 'x-ledger-generated-at', 'x-liveness-generated-at', 'x-index-db-sha256', 'retry-after'];",
  ...
  "  var mapP = mkEl('p', 'status');",
  "  var mapLink = document.createElement('a');",
  "  mapLink.href = '#map?did=' + encodeURIComponent(body.did);",
  "  mapLink.textContent = '→ liveness tier for this DID';",
  "  mapP.appendChild(mapLink);",
  "  wrap.appendChild(mapP);",
  ...
  "  });",
  "}",
  "function sumCounts(obj) { var total = 0; var keys = Object.keys(obj); for (var i = 0; i < keys.length; i++) total += Number(obj[keys[i]]) || 0; return total; }",
  "function orderedChips(counts, order) { var wrap = mkEl('p'); for (var i = 0; i < order.length; i++) { var k = order[i]; wrap.appendChild(mkText('span', k + ' ' + fmtNum(counts[k] || 0), 'kind')); } return wrap; }",
  "var MAP_CLASS_ORDER = ['live', 'mixed', 'quiet', 'unknown', 'farm', 'flood'];",
  "function mapClassRank(c) { var i = MAP_CLASS_ORDER.indexOf(c); return i === -1 ? MAP_CLASS_ORDER.length : i; }",
  "function fmtPoints(n) { var x = Number(n); return (x > 0 ? '+' : '') + String(x); }",
  "var MAP_POINT_RULES = {",
  "  work_cycles_ge_1: { signal: 'work_cycles', rule: '>= 1' },",
  "  work_cycles_ge_5: { signal: 'work_cycles', rule: '>= 5' },",
  "  reply_in_nonburst_ge_1: { signal: 'reply_in_nonburst', rule: '>= 1 (distinct non-burst DIDs addressing this one)' },",
  "  reply_in_nonburst_ge_3: { signal: 'reply_in_nonburst', rule: '>= 3' },",
  "  reply_out_ge_1: { signal: 'reply_out', rule: '>= 1' },",
  "  reply_out_ge_5: { signal: 'reply_out', rule: '>= 5' },",
  "  github_contrib_ge_1: { signal: 'github_contrib_rows', rule: '>= 1' },",
  "  live_rooms_ge_2: { signal: 'live_rooms_count', rule: '>= 2 (rooms classed live or mixed)' },",
  "  did_note_present: { signal: 'did_note_present', rule: 'is true' },",
  "  distinct_ge_0_8_and_posts_ge_3: { signal: 'distinct_text_ratio', rule: '>= 0.8, and post_count >= 3' },",
  "  age_ge_7d: { signal: 'age_days', rule: '>= 7 (days since the ledger first saw this DID)' },",
  "  faucet_onboarding_majority: { signal: 'faucet_onboarding_rows', rule: '>= majority of post_count' },",
  "  template_majority_and_posts_ge_3: { signal: 'template_rows', rule: '>= majority of post_count, and post_count >= 3' },",
  "  one_line: { signal: 'post_count', rule: '== 1' },",
  "};",
  "function renderMapStatsCards(body) {",
  "  var grid = mkEl('div', 'stats');",
  "  grid.appendChild(statCard('Map generated', fmtAge(body.generated_at), fmtAbs(body.generated_at) + ' · window ' + body.window_days + ' d · ' + fmtNum(body.log_rows) + ' log rows'));",
  "  grid.appendChild(statCard('Ledger joined', fmtAge(body.ledger_generated_at), fmtAbs(body.ledger_generated_at)));",
  "  grid.appendChild(statCard('Rooms', fmtNum(sumCounts(body.counts.rooms_by_class)), orderedChips(body.counts.rooms_by_class, MAP_CLASS_ORDER)));",
  "  grid.appendChild(statCard('Agents', fmtNum(sumCounts(body.counts.agents_by_tier)), orderedChips(body.counts.agents_by_tier, ['live', 'likely_live', 'weak', 'farm', 'unknown'])));",
  "  return grid;",
  "}",
  "function jumpToRoom(name) { byId('map-room').value = name; byId('map-did').value = ''; runMap(true); }",
  "function renderMapRoomTable(rooms, filterClass) {",
  "  var names = Object.keys(rooms);",
  "  names.sort(function (a, b) { var ra = mapClassRank(rooms[a].class); var rb = mapClassRank(rooms[b].class); if (ra !== rb) return ra - rb; return a < b ? -1 : a > b ? 1 : 0; });",
  "  var rows = [];",
  "  for (var i = 0; i < names.length; i++) {",
  "    var name = names[i];",
  "    var room = rooms[name];",
  "    if (filterClass && filterClass !== 'all' && room.class !== filterClass) continue;",
  "    var w = room.facts.window;",
  "    var chip = mkText('button', name, 'chip');",
  "    chip.type = 'button';",
  "    (function (theRoom) { chip.addEventListener('click', function () { jumpToRoom(theRoom); }); })(name);",
  "    rows.push(trow([",
  "      td(chip),",
  "      td(room.class),",
  "      td(room.class_all),",
  "      td(String(w.distinct_senders), 'num'),",
  "      td(String(w.reply_senders) + ' / ' + String(w.reply_sender_share), 'num'),",
  "      td(String(w.template_sender_share), 'num'),",
  "      td(String(w.one_line_sender_share), 'num'),",
  "      td(String(w.top_sender_share), 'num'),",
  "      td(String(w.rows_per_5min_p95), 'num'),",
  "      td(w.last_ts ? fmtWhen(w.last_ts) : '—'),",
  "    ]));",
  "  }",
  "  return buildTable(['room', 'class', 'class_all', 'senders', 'reply senders / share', 'template share', 'one-line share', 'top-sender share', 'rows/5min p95', 'last activity'], rows);",
  "}",
  "function renderMapResultsBlock(body) {",
  "  var wrap = byId('map-results');",
  "  clearNode(wrap);",
  "  wrap.appendChild(renderMapStatsCards(body));",
  "  wrap.appendChild(mkText('h3', 'rooms'));",
  "  wrap.appendChild(renderMapRoomTable(body.rooms, byId('map-class').value));",
  "}",
  "function renderMapOverview(body) {",
  "  MAP_OVERVIEW = body;",
  "  renderMapResultsBlock(body);",
  "  var methodWrap = byId('map-method');",
  "  clearNode(methodWrap);",
  "  var details = mkEl('details');",
  "  details.appendChild(mkText('summary', 'Method (verbatim from the map)'));",
  "  var pre = mkEl('pre');",
  "  pre.textContent = JSON.stringify(body.method, null, 2);",
  "  details.appendChild(pre);",
  "  methodWrap.appendChild(details);",
  "}",
  "function renderMapRoomResult(status, body) {",
  "  var wrap = byId('map-results');",
  "  clearNode(wrap);",
  "  if (typeof body.error === 'string') {",
  "    if (body.error === 'unknown_room') {",
  "      var genAt = CARD && CARD.liveness ? CARD.liveness.generated_at : 'an unknown map';",
  "      wrap.appendChild(mkText('p', 'unknown_room — well-formed room id, not in the map built ' + genAt + ' — absence is not evidence', 'status'));",
  "    } else {",
  "      wrap.appendChild(mkText('p', status + ' ' + body.error, 'status error'));",
  "    }",
  "    return;",
  "  }",
  "  wrap.appendChild(mkText('p', 'room: ' + body.room + ' · class ' + body.class + ' (all-time ' + body.class_all + ')', 'status'));",
  "  wrap.appendChild(mkText('h3', 'decided_on'));",
  "  var rows = [];",
  "  for (var i = 0; i < body.decided_on.length; i++) {",
  "    var triple = body.decided_on[i];",
  "    rows.push(trow([td(String(triple[0]), 'key'), td(String(triple[1]), 'num'), td(String(triple[2]))]));",
  "  }",
  "  wrap.appendChild(buildTable(['fact', 'value', 'threshold'], rows));",
  "  wrap.appendChild(mkText('h3', 'facts (window)'));",
  "  wrap.appendChild(renderFactsTable(body.facts.window));",
  "  wrap.appendChild(mkText('h3', 'facts (all time)'));",
  "  wrap.appendChild(renderFactsTable(body.facts.all));",
  "}",
  "function renderMapAgentResult(status, body) {",
  "  var wrap = byId('map-results');",
  "  clearNode(wrap);",
  "  if (typeof body.error === 'string') {",
  "    if (body.error === 'unknown_agent') {",
  "      var n = CARD && CARD.liveness ? fmtNum(CARD.liveness.agents) : 'an unknown number of';",
  "      var genAt2 = CARD && CARD.liveness ? CARD.liveness.generated_at : 'an unknown map';",
  "      wrap.appendChild(mkText('p', 'unknown_agent — not among the ' + n + ' agents in the map built ' + genAt2 + ' — absence is not evidence', 'status'));",
  "    } else {",
  "      wrap.appendChild(mkText('p', status + ' ' + body.error, 'status error'));",
  "    }",
  "    return;",
  "  }",
  "  wrap.appendChild(mkText('span', 'tier: ' + body.tier, 'tag'));",
  "  wrap.appendChild(statCard('points', String(body.points), 'did: ' + body.did));",
  "  var rows = [];",
  "  var pointKeys = Object.keys(body.thresholds.agent_points).sort();",
  "  for (var i = 0; i < pointKeys.length; i++) {",
  "    var key = pointKeys[i];",
  "    var info = MAP_POINT_RULES[key] || { signal: null, rule: key };",
  "    var value = info.signal && body.signals[info.signal] !== undefined ? String(body.signals[info.signal]) : '—';",
  "    rows.push(trow([td(key, 'key'), td(value, 'num'), td(info.rule), td(fmtPoints(body.thresholds.agent_points[key]), 'num')]));",
  "  }",
  "  wrap.appendChild(mkText('h3', 'signals · rule · points (every marker the tier is decided on, with the number beside its rule)'));",
  "  wrap.appendChild(buildTable(['signal', 'value', 'rule', 'points'], rows));",
  "  wrap.appendChild(mkText('p', 'unsigned_rows ' + String(body.signals.unsigned_rows) + ' — informational, never a point', 'status'));",
  "  wrap.appendChild(mkText('p', 'tier thresholds: live ≥ ' + body.thresholds.tier_thresholds.live + ' · likely_live ≥ ' + body.thresholds.tier_thresholds.likely_live + ' · weak ≥ ' + body.thresholds.tier_thresholds.weak + ' · farm ≤ 0 with a negative marker · unknown otherwise', 'status'));",
  "}",
  "function mapArgsFromInputs() { return { room: byId('map-room').value, did: byId('map-did').value }; }",
  "function runMap(scroll) {",
  "  var args = mapArgsFromInputs();",
  "  var btn = byId('map-submit');",
  "  var statusEl = byId('map-status');",
  "  var resultsEl = byId('map-results');",
  "  setStatus(statusEl, 'loading…');",
  "  if (scroll) { var sec = byId('map'); if (sec.scrollIntoView) sec.scrollIntoView(); }",
  "  if (args.did) {",
  "    var didParams = new URLSearchParams();",
  "    didParams.set('did', args.did);",
  "    updateFragment('map', didParams);",
  "    var didUrl = location.origin + '/liveness/agent/' + encodeURIComponent(args.did);",
  "    runRequest('map', didUrl, btn).then(function (result) {",
  "      if (result.kind === 'ok' || result.kind === 'app_error') {",
  "        setStatus(statusEl, '');",
  "        safeRender(statusEl, resultsEl, function () { renderMapAgentResult(result.info.status, result.info.parsed); });",
  "      } else {",
  "        setStatus(statusEl, errorMessage(result), true);",
  "        clearNode(resultsEl);",
  "      }",
  "      var wrap = byId('map-raw');",
  "      clearNode(wrap);",
  "      wrap.appendChild(buildRawBlock(result.info, buildCurl(didUrl), buildMcpCall('liveness', { did: args.did }), 'POST ' + location.origin + '/mcp · tools/call liveness'));",
  "    });",
  "    return;",
  "  }",
  "  if (args.room) {",
  "    var roomParams = new URLSearchParams();",
  "    roomParams.set('room', args.room);",
  "    updateFragment('map', roomParams);",
  "    var roomUrl = location.origin + '/liveness/room/' + encodeURIComponent(args.room);",
  "    runRequest('map', roomUrl, btn).then(function (result) {",
  "      if (result.kind === 'ok' || result.kind === 'app_error') {",
  "        setStatus(statusEl, '');",
  "        safeRender(statusEl, resultsEl, function () { renderMapRoomResult(result.info.status, result.info.parsed); });",
  "      } else {",
  "        setStatus(statusEl, errorMessage(result), true);",
  "        clearNode(resultsEl);",
  "      }",
  "      var wrap = byId('map-raw');",
  "      clearNode(wrap);",
  "      wrap.appendChild(buildRawBlock(result.info, buildCurl(roomUrl), buildMcpCall('liveness', { room: args.room }), 'POST ' + location.origin + '/mcp · tools/call liveness'));",
  "    });",
  "    return;",
  "  }",
  "  updateFragment('map', null);",
  "  var overviewUrl = location.origin + '/liveness';",
  "  runRequest('map', overviewUrl, btn).then(function (result) {",
  "    if (result.kind === 'ok') {",
  "      setStatus(statusEl, '');",
  "      safeRender(statusEl, resultsEl, function () { renderMapOverview(result.info.parsed); });",
  "    } else {",
  "      setStatus(statusEl, errorMessage(result), true);",
  "      clearNode(resultsEl);",
  "      MAP_OVERVIEW = null;",
  "      clearNode(byId('map-method'));",
  "    }",
  "    var wrap = byId('map-raw');",
  "    clearNode(wrap);",
  "    wrap.appendChild(buildRawBlock(result.info, buildCurl(overviewUrl), buildMcpCall('liveness', {}), 'POST ' + location.origin + '/mcp · tools/call liveness'));",
  "  });",
  "}",
  ...
  "}",
  "var PANEL_PARAM_NAMES = { search: ['q', 'k', 'kind'], did: ['did'], route: ['model_hash', 'precision', 'max_latency_ms', 'k'], health: [], map: ['room', 'did'] };",
  "var PANEL_REQUIRED = { search: 'q', did: 'did', route: 'model_hash', health: null, map: null };",
  "var PANEL_INPUT_IDS = {",
  ...
  "  else if (panel === 'map') runMap(false);",
  "}",
  ...
  "  byId('map-form').addEventListener('submit', function (ev) { ev.preventDefault(); runMap(false); });",
  "  byId('map-class').addEventListener('change', function () { if (MAP_OVERVIEW) renderMapResultsBlock(MAP_OVERVIEW); });",
...
const MAP_MARKUP =
  '<section id="map">' +
  sectionHead("05", "Map", "GET /liveness · MCP tool liveness") +
  '<form id="map-form">' +
  '<div class="f grow"><label for="map-room">room</label>' +
  '<input type="text" id="map-room" name="room" maxlength="128">' +
  '<span class="hint">leave room and did both empty for the overview</span></div>' +
  '<div class="f grow"><label for="map-did">did</label>' +
  '<input type="text" id="map-did" name="did" maxlength="200"></div>' +
  '<div class="f"><label for="map-class">class filter</label>' +
  '<select id="map-class"><option value="all">all</option><option value="live">live</option>' +
  '<option value="mixed">mixed</option><option value="quiet">quiet</option>' +
  '<option value="unknown">unknown</option><option value="farm">farm</option>' +
  '<option value="flood">flood</option></select></div>' +
  '<button type="submit" id="map-submit">Run</button>' +
  "</form>" +
  '<p id="map-status" class="status" aria-live="polite"></p>' +
  '<div id="map-results"></div>' +
  '<div id="map-method"></div>' +
  '<p class="caveat">A class or tier is evidence from one log, never a verdict about a person -- computed the same deterministic way for every room and DID from published thresholds over published counted facts.</p>' +
  '<p class="caveat">`unknown` is the honest default, not a negative signal -- too little evidence (below the published room/agent minimums) is `unknown`, never `live` or `farm`.</p>' +
  '<p class="caveat">This map is not a ban list, and not a truth about anyone -- nothing here blocks, filters, or claims to identify a real-world operator behind an identity.</p>' +
  '<p class="caveat">As in the DID panel above, no signature verification happens anywhere upstream of this map -- a `sig`/`signed` field is a recorded fact, never a cryptographic check.</p>' +
  '<div id="map-raw"></div>' +
  "</section>";

const BODY_MARKUP = TOPBAR_MARKUP + HEADER_MARKUP + SEARCH_MARKUP + DID_MARKUP + ROUTE_MARKUP + HEALTH_MARKUP + MAP_MARKUP + FOOTER_MARKUP;
