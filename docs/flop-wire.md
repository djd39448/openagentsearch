...
`tests/fixtures/flop/wire-format-v1-report.json` is a committed, byte-deterministic report of
every check `tests/test_flop_wire_corpus.py` performs against the corpus above, generated by
`scripts/make_wire_conformance_report.py`. It exists so a third party --
`retardio73-boop/flop-conformance-lab`'s #58 checklist asks for exactly this -- can diff their own
run of the script against ours without re-deriving anything: the exact script, the full output,
the pinned upstream commit, the corpus sha256, and the regenerate command all travel together in
one file (the `regenerate` field).

Regenerate it with:

```
python scripts/make_wire_conformance_report.py --corpus tests/fixtures/flop/wire-format-v1.json --out tests/fixtures/flop/wire-format-v1-report.json
```

`tests/test_wire_conformance_report.py` fails if the committed file is stale (a subprocess run of
the script must reproduce it byte-for-byte), and separately checks the summary counts
(`negative_cases: 14`, `deviations: 1`, `not_verifiable: 3`, `matching: 10`), the corpus sha256 pin, and that every
corpus `negative_cases[*].id` appears in the report exactly once.

The report's `negative_cases[*].observed` is one of three shapes: `"rejected: <ReasonName>"` (the
decoder raised a `WireDecodeError`, or a `verify.SignatureCheck` came back `"rejected"`, with that
exact reason), `"not_rejected: <why>"` (no rejection occurred -- either because the corpus itself
does not expect one, like `wrong_genesis_network`/`wrong_session`'s "different channel_id", or
because of the known `wrong_path_orientation` corpus defect below), or `"not_verifiable: <why>"`.

**`not_verifiable` means exactly what it says, not "probably fine."** Every case whose expected
outcome is a signature verdict -- `invalid_receipt_signature`, `invalid_validator_signature` and
`legacy_receipt_current_channel` (expected `BadReceiptSignature`) -- is always reported
`not_verifiable` with `matches_expected: null`, even though `tests/test_flop_wire_corpus.py` itself
proves the corresponding rejection using an injected `StubVerifier`. What IS checkable is kept in
the case's `note` (for the legacy receipt: it decodes as the untagged 96-byte message, equal to the
v1 message without its 29-byte domain/version prefix). The report generator deliberately does not run that stub: a stub
verifier is a test device, not an sr25519 implementation, and this report never claims a
cryptographic result this package cannot actually produce (see "What is NOT verified" above). What
it does report is what production code (no injected verifier) actually returns for these three
cases -- always `status="not_verified"` -- which is evidence of the *seam's* behavior, not of the
signature's validity.

`wrong_path_orientation` is reported `not_rejected` with `matches_expected: false` and mirrored
into the top-level `deviations` array; see "One documented corpus discrepancy" below for why this
specific mutation cannot be detected by any implementation of the documented Merkle-walk algorithm.

**This report is evidence, not certification.** It is a description of what this package's
decoders do against one pinned, offline corpus snapshot on the machine that ran it -- not a claim
that FLOP v1 wire objects seen in the wild will decode the same way, and not a substitute for
reading `tests/test_flop_wire_corpus.py` yourself.
