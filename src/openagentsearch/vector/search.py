...
def cosine_search(store: VectorStore, query_vector: List[float], k: int) -> List[Tuple[str, float]]:
    ...
    Raises:
        ValueError: if k <= 0; if the query vector is empty, all zeros, or contains boolean,
            non-numeric or non-finite (NaN/infinite) elements; if any stored vector does not have
            the query's dimension or contains a non-finite element; or if a magnitude or score
            overflows to a non-finite value. A non-finite score is never returned.
    """
    if k <= 0:
    ...
    for i, value in enumerate(query_vector):
        ...
        if not math.isfinite(float(value)):
            raise ValueError(f"query vector element at index {i} is not finite: {value!r}")
    ...
    if not math.isfinite(query_magnitude):
        raise ValueError("query vector magnitude overflowed to a non-finite value")

    records = store.load_all()

    # Validate every stored vector before producing any result. A real VectorStore already
    # rejects corrupt persisted rows; the checks stay here so any store-compatible object is safe.
    for record in records:
        ...
        for i, value in enumerate(stored):
            if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)):
                raise ValueError(
                    f"stored vector for {record['chunk_id']} has a non-finite or non-numeric "
                    f"element at index {i}: {value!r}"
                )
    ...
    for record in records:
        ...
        if not math.isfinite(stored_magnitude):
            raise ValueError(
                f"stored vector for {record['chunk_id']} has a magnitude that overflowed to a non-finite value"
            )
        if stored_magnitude == 0.0:
            ...
            if not math.isfinite(score):
                raise ValueError(
                    f"cosine score for {record['chunk_id']} is not finite"
                )
